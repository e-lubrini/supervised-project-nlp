...
second_node = ['grocery', 'beauty', 'magazines', 'jewelry_watches', 'sports_outdoors', 'cell_phones_service']
...
def test_stacking(model1, model2, model3, meta):
    model1.eval()
    model2.eval()
    model3.eval()
    correct = 0.
    ...
    for data, target in test_loader:
        ...
        pred = predict.tolist()
        predict = torch.tensor(meta.predict_proba(pred), device='cuda')
        _, acc = criterion(predict, target)
        amount = len(target) - support * 2
        correct += acc * amount
        count += amount
...
test_stacking(model1, model2, model3, lr)
